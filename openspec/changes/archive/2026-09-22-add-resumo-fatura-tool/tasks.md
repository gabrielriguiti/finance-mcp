...
- [x] 4.1 Validar `resumo_fatura` no Claude Desktop (já configurado via `claude_desktop_config.json`): fatura com transações no período, conta inexistente, conta corrente (não-cartão), e um `mes_referencia` em que o vencimento cai no mês seguinte ao fechamento.
