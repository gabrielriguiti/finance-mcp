...
- [ ] 2.1 Criar `src/db/connection.ts` exportando uma conexão `better-sqlite3` única para `finance.db`. Verificar com um import de teste que a conexão abre sem erro.

## 3. Tool get_saldo

- [ ] 3.1 Criar `src/tools/get-saldo.ts` com schema zod (`conta` opcional, string) e handler que calcula saldo_inicial + soma de transações não pendentes, por conta ou para todas as contas. Verificar com uma chamada direta do handler (script ou teste) cobrindo: conta existente, sem `conta`, e transação pendente não somada.
- [ ] 3.2 Implementar retorno de erro estruturado (`isError: true`) quando `conta` informada não existe. Verificar chamando o handler com um nome de conta inexistente.
